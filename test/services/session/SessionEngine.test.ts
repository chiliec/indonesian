...
beforeEach(async () => {
  ...
  await users.touchUser(1, { defaultLocale: 'en' });
...
test('start creates an active session and renders the first question', async () => {
  const r = await eng.start(1, 100, 'mixed', true, () => 0.5);
  assert.ok(r);
...
test('correct choice answer advances with flash and awards xp', async () => {
  await eng.start(1, 100, 'mixed', true, () => 0.5);
  const s = (await sessions.findActive(1))!;
  const ex = s.exercises[0]!;
  const out = await eng.submitChoice(1, s._id.toString(), ex.correctIndex!, true);
  assert.ok(out);
...
test('wrong choice answer shows feedback and requeues the card once', async () => {
  ...
  const out = await eng.submitChoice(1, s._id.toString(), wrongIdx, true);
  ...
  const next = await eng.next(1, s._id.toString(), true);
...
test('stale session id returns null', async () => {
  await eng.start(1, 100, 'mixed', true, () => 0.5);
  assert.equal(await eng.submitChoice(1, 'f'.repeat(24), 0, true), null);
});
...
test('finishing the last question completes with bonus and review list', async () => {
  ...
  for (let i = 0; i < s.exercises.length; i++) {
    ...
    out = await eng.submitChoice(1, s._id.toString(), ex.correctIndex!, true);
  }
...
test('start abandons a prior active session', async () => {
  const a = await eng.start(1, 100, 'mixed', true, () => 0.5);
  await eng.start(1, 100, 'mixed', true, () => 0.5);
